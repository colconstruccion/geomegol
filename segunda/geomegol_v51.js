...
const players = document.querySelectorAll('.input-grid input');
  ...
  if (i % 2 === 0){
    ...
    if(i % 2 !== 0){
      players[i].addEventListener("change",function(){
        ...
        if (ocupada == true){
          instructionsCard.innerHTML = "<h2>Amarilla para el equipo local</h2>";
          ctx.beginPath();
...
function iniciarVisitantes(){
  ...
  for(let i=22;i<players.length;i+=2){
    let coor_x = playersData.defensa[n].x;
    let coor_y = playersData.defensa[n].y;
...
function moverBalon(){
    ...
    if (Math.abs(gol_delta_y) <= 25 && Math.abs(gol_delta_x) <= 10){
          ...
          flashColors(0.5);
          flashColors(3);
          //revisar si locales va ganando el juego
...
for (let i=0; i<players.length; i+=2){
    players[i].addEventListener("click",function(){
      ...
      players[j].addEventListener("change",function(){
        ...
        if (prevCoor_y !== "" && !isNaN(prevCoor_x)){
          ...
          isMoving = moverJugador();
        }
        // dibujar la cancha de nuevo
        if (isMoving == "false"){
          drawGoalArea('left');
          drawGoalArea('right');
          dibujarBalon();
  ...
  function moverJugador(){
    ...
    //let con_delta_x = Math.abs(delta_x);
    //let con_delta_y = Math.abs(delta_y);
    // borrar la pelota
        ctx.beginPath();
        ...
        //dibujar jugador local
          ...
          drawCenterLineAndCircle();
          //dibujarBalon();
        if(delta_x * delta_x + delta_y * delta_y > 144){
